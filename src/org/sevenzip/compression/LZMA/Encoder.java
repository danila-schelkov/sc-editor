...
public class Encoder {
	...
	static class LiteralEncoder {
		static class Encoder2 {
			...
			public void Init() { org.sevenzip.compression.RangeCoder.Encoder.initBitModels(m_Encoders); }



			public void encode(org.sevenzip.compression.RangeCoder.Encoder rangeEncoder, byte symbol) throws IOException {
				int context = 1;
				for (int i = 7; i >= 0; i--) {
					int bit = ((symbol >> i) & 1);
			...
			public void encodeMatched(org.sevenzip.compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException {
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--) {
					int bit = ((symbol >> i) & 1);
			...
			public int getPrice(boolean matchMode, byte matchByte, byte symbol) {
				...
				if (matchMode) {
					for (; i >= 0; i--) {
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += org.sevenzip.compression.RangeCoder.Encoder.getPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit) {
							i--;
							break;
				...
				for (; i >= 0; i--) {
					int bit = (symbol >> i) & 1;
					price += org.sevenzip.compression.RangeCoder.Encoder.getPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
		...
		public Encoder2 getSubCoder(int pos, byte prevByte) { return m_Coders[((pos & m_PosMask) << m_NumPrevBits) + ((prevByte & 0xFF) >>> (8 - m_NumPrevBits))]; }
	}
	...
	static class LenEncoder {
		...
		public void init(int numPosStates) {
			org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_choice);
		...
		public void encode(org.sevenzip.compression.RangeCoder.Encoder rangeEncoder, int symbol, int posState) throws IOException {
		...
		public void setPrices(int posState, int numSymbols, int[] prices, int st) {
			int a0 = org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_choice[0]);
			int a1 = org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_choice[0]);
			int b0 = a1 + org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_choice[1]);
			int b1 = a1 + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_choice[1]);
	...
	org.sevenzip.compression.RangeCoder.Encoder _rangeEncoder = new org.sevenzip.compression.RangeCoder.Encoder();
	...
	void init() {
		...
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isMatch);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isRep0Long);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isRep);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isRepG0);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isRepG1);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_isRepG2);
		org.sevenzip.compression.RangeCoder.Encoder.initBitModels(_posEncoders);
	...
	int GetRepLen1Price(int state, int posState) {
		return org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRepG0[state]) +
				org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}

	int GetPureRepPrice(int repIndex, int state, int posState) {
		int price;
		if (repIndex == 0) {
			price = org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRepG0[state]);
			price += org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else {
			price = org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRepG1[state]);
			else {
				price += org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRepG1[state]);
				price += org.sevenzip.compression.RangeCoder.Encoder.getPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int getOptimum(int position) throws IOException {
		...
		_optimum[1].Price = org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.getSubCoder(position, _previousByte).getPrice(!Base.stateIsCharState(_state), matchByte, currentByte);
		_optimum[1].makeAsChar();

		int matchPrice = org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRep[_state]);
		...
		while (true) {
			...
			int curAnd1Price = curPrice +
				org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte) {
				...
				if (lenTest2 >= 2) {
					...
					int nextRepMatchPrice = curAnd1Price +
						org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRep[state2]); {
				...
				if (lenTest < numAvailableBytesFull) {
					...
					if (lenTest2 >= 2) {
						...
						int curAndLenCharPrice =
								...
								org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice1(_isRep[state2]);
			...
			if (newLen >= startLen) {
				normalMatchPrice = matchPrice + org.sevenzip.compression.RangeCoder.Encoder.getPrice0(_isRep[state]);
	...
	void writeEndMarker(int posState) throws IOException {
		...
		_lenEncoder.encode(_rangeEncoder, 0, posState);
		int posSlot = (1 << Base.kNumPosSlotBits) - 1;
		int lenToPosState = Base.getLenToPosState(Base.kMatchMinLen);
	...
	void setOutStream(java.io.OutputStream outStream) { _rangeEncoder.setStream(outStream); }
	...
	public void code(java.io.InputStream inStream, java.io.OutputStream outStream, ICodeProgress progress) throws IOException {
		_needReleaseMFStream = false;
		try {
	...
	void fillDistancesPrices() {
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << org.sevenzip.compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
