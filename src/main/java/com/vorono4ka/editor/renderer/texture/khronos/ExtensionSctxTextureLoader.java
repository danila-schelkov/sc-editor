...
public class ExtensionSctxTextureLoader implements SctxTextureLoader {
    ...
    @Override
    public void load(Texture texture, SctxTexture sctxTexture) {
        ...
        if (pixelType == PixelType.UNCOMPRESSED) {
            ...
            for (int level = 0; level < mipMaps.size(); level++) {
                MipMapData mipMap = mipMaps.get(level);
                texture.initCompressed(level, internalFormat, format, sctxTexture.getWidth(), sctxTexture.getHeight(), BufferUtils.wrapDirect(mipMap.data()));
            }
        }
    }
